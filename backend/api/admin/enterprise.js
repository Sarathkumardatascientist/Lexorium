...
const ENTERPRISE_COLLECTION = 'enterprise_users';
const ADMIN_DOC = 'admin_config';
...
async function getAdminConfig() {
  try {
    const db = getDb();
    const doc = await db.collection(ENTERPRISE_COLLECTION).doc(ADMIN_DOC).get();
    if (!doc.exists) {
...
async function saveAdminConfig(config) {
  try {
    const db = getDb();
    await db.collection(ENTERPRISE_COLLECTION).doc(ADMIN_DOC).set(config);
    return true;
  } catch (err) {
...
async function validateSession(token) {
  ...
  if (Date.now() > session.expiresAt) {
    ...
    await saveAdminConfig({ passwordHash: config.passwordHash, sessions: sessions });
    return false;
  }
  return true;
}
...
async function getEnterpriseUsers() {
  try {
    const db = getDb();
    const snapshot = await db.collection(ENTERPRISE_COLLECTION).where('_type', '==', 'user').get();
    const users = [];
    snapshot.forEach(doc => {
      const data = doc.data();
      if (data.email) {
        users.push({ email: data.email, addedAt: data.addedAt, addedBy: data.addedBy });
      }
    });
    return users;
  } catch (err) {
...
async function addEnterpriseUser(email) {
  try {
    const db = getDb();
    await db.collection(ENTERPRISE_COLLECTION).add({
      _type: 'user',
      email: email,
    ...
    console.error('Error adding enterprise user:', err);
    return false;
  }
}

async function removeEnterpriseUser(email) {
  try {
    const db = getDb();
    const snapshot = await db.collection(ENTERPRISE_COLLECTION)
      .where('_type', '==', 'user')
      .where('email', '==', email)
      .get();
    ...
    console.error('Error removing enterprise user:', err);
    return false;
  }
}
...
async function handleAdminEnterprise(req, res) {
  ...
  if (parsed.action === 'setup') {
    ...
    await saveAdminConfig({ passwordHash: hash, sessions: {} });
  ...
  if (parsed.action === 'login') {
    ...
    if (!verifyPassword(parsed.password, config.passwordHash)) {
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.statusCode = 401;
      return res.end('<html><body style="font-family:Arial,sans-serif;background:#0A0A0A;color:#fff;display:flex;justify-content:center;align-items:center;min-height:100vh;margin:0"><div style="background:#1F1F1F;padding:40px;border-radius:16px;width:400px;text-align:center"><h1 style="font-size:24px;margin:0 0 8px;font-style:italic">Lexorium <span style="color:#8B5CF6">Enterprise</span></h1><p style="color:#888;margin-bottom:24px">Admin Login</p><p style="color:#f87171;margin-bottom:16px;font-size:13px">Invalid password</p><form method="POST" action="/api/admin/enterprise/login"><input type="password" name="password" placeholder="Password" required style="width:100%;padding:14px;border-radius:10px;border:1px solid #444;background:#2a2a2a;color:#fff;font-size:14px;box-sizing:border-box;margin-bottom:16px"><button type="submit" style="width:100%;padding:14px;border-radius:10px;border:none;background:linear-gradient(135deg,#8B5CF6,#6366F1);color:white;font-size:14px;cursor:pointer">Login</button></form></div></body></html>');
    }
    const loginToken = await createSession();
  ...
  if (parsed.action === 'logout') {
    ...
    await saveAdminConfig({ passwordHash: cfg.passwordHash, sessions: sess });
  ...
  if (action === 'remove') {
    await removeEnterpriseUser(normalizedEmail);
    return res.json({ ok: true, message: normalizedEmail + ' removed from Enterprise' });
  }

  if (action === 'add' || !action) {
    await addEnterpriseUser(normalizedEmail);
    return res.json({
      ok: true,
